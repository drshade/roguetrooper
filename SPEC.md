...
- **Should**: Several companions exist by demo: a **homing-missile** companion (targets a randomly selected live enemy), a **shotgun** companion (targets the closest enemy, firing a burst of scattered pellets), a **repulsor-field** companion (emits an expanding wave that knocks enemies radially outward, dealing no damage — pure crowd control), a **boomerang** companion (flings a large boomerang in a random direction that sweeps a wide S across the field and returns, carving every enemy it crosses — untargeted area coverage), and a **tesla-coil** companion (on a slow cooldown, strikes a randomly selected enemy within its long range with lightning that then arcs to the nearest not-yet-struck troopers in bounce range — max 2 bounces, no knockback).
