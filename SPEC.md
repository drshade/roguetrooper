...
- **Should**: Several companions exist by demo: a **homing-missile** companion (targets a randomly selected live enemy), a **shotgun** companion (targets the closest enemy, firing a burst of scattered pellets), a **repulsor-field** companion (emits an expanding wave that knocks enemies radially outward, dealing no damage — pure crowd control), and a **boomerang** companion (flings a large boomerang in a random direction that sweeps a wide S across the field and returns, carving every enemy it crosses — untargeted area coverage).
